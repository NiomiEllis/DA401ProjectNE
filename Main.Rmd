...
## Extra Visuals & Credibility Checks



### Small multiples panel (Coefficint and T-value side by side)

```{r}
# Reshape the GWR results
panel_df <- gwr_data %>%
  dplyr::select(marginalized_index, marginalized_index_TV, geometry) %>%
  pivot_longer(
    cols = -geometry,
    names_to = "variable",
    values_to = "value"
...
ggplot(panel_df) +
  ...
  theme_minimal() +
  labs(
    title = "GWR Coefficient and Local t-values for Marginalization Index",
    fill  = ""
  )
...
ggplot() +
  geom_sf(data = gwr_data, fill = "gray95", color = NA) +
  geom_point(
    data = centroids,
    aes(
      x = st_coordinates(centroids)[,1],
      y = st_coordinates(centroids)[,2],
      color = marginalized_index,
      size = abs(marginalized_index_TV)
    ),
    alpha = 0.7
  ) +
  ...
  theme_minimal() +
  labs(
    title = "GWR Results: Coefficients (color) and t-values (size)",
    color = "Coefficient",
    size = "|t-value|"
  )
