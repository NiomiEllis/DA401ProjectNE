...
#Making the base theme for all visuals for readability
# --------------------------------------------------
...
p_poverty <- ggplot() +
  geom_sf(data = counties_plot, aes(fill = poverty_rate), color = NA) +
  geom_sf(data = states_sf, fill = NA, color = "black", size = 0.5) +
  scale_fill_viridis_c(option = "cividis", name = "Poverty Rate (%)") +
...
#  THEME FOR READABILITY (BOLD, DARK TEXT)
# --------------------------------------------------
theme_gwr <- theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(),
...
ggsave(
  file.path(fig_dir, "GWR_direct_marginalized_coef_map.png"),
  plot = p_coef,
  width = 11,      
  height = 9,
  dpi = 320        
...
# Save Local R map
ggsave(
  file.path(fig_dir, "GWR_direct_localR2_map.png"),
  plot = p_r2,
  width = 11,
  height = 9,
  dpi = 320
